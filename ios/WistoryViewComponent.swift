...
@objc class WistoryViewComponent: UIView {
    let wistory = RNWistoryConfig.sharedInstance()?.apiUrl ?
        Wistory(with: RNWistoryConfig.sharedInstance()?.companyToken ?? "", userToken: RNWistoryConfig.sharedInstance()?.userToken, usageSettings: .embedded)
            .customBaseURL(RNWistoryConfig.sharedInstance()?.apiUrl)
